...
import { TABLES, getTable, rowKey, rowLabel, type ColumnDef, type Row, type TableData } from "./schema";
...
interface TableBrowserProps {
  ...
  onDataChange: (next: TableData) => void;
}

export function TableBrowser({ dataByTable, onDataChange }: TableBrowserProps) {
  ...
  function handleDelete(key: string) {
    onDataChange({
      ...dataByTable,
      [selectedTableName]: (dataByTable[selectedTableName] ?? []).filter((r) => rowKey(table, r) !== key),
    });
    setConfirmDeleteKey(null);
  }
                        ...
                        <tr className="dp-confirm-row">
                          <td colSpan={table.columns.length + 1}>
                            <div className="dp-confirm-inner">
                              ...
                              <button type="button" className="btn-action btn-action--danger" onClick={() => handleDelete(key)}>
                                Yes, delete
                              </button>
