...
interface MetricFormModalProps {
  ...
  initialValues?: MetricFormInitialValues | null;
  onClose: () => void;
  onSaved: (metric: TrackedMetric, isNew: boolean) => void;
}

export interface MetricFormInitialValues {
  label: string;
  key?: string;
  unitText?: string;
  timeSpan?: MetricTimeSpan;
  timeSpanCustomText?: string | null;
  target?: number | null;
...
function autoGeneratedKey(label: string, preferredKey?: string | null, attempt = 0): string {
  const preferredBase = preferredKey ? slugify(preferredKey) : "";
  const labelBase = slugify(label);
  const base = preferredBase || labelBase || "metric";
...
function isKeyConflictError(err: ApiError): boolean {
  if (err.status === 409) return true;
  if (Boolean(err.fieldErrors?.key)) return true;
  ...
  useEffect(() => {
    if (show) {
      if (metric) {
        ...
        setUnitText((initialValues?.unitText ?? "count").toString());
        setTimeSpan(initialValues?.timeSpan ?? "day");
        setTimeSpanCustomText(initialValues?.timeSpanCustomText ?? "");
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      if (isEdit && metric) {
        ...
        const createWithAutoKey = async (): Promise<TrackedMetric> => {
          const preferredKey = initialValues?.key ?? null;
          let lastKeyError: ApiError | null = null;

          for (let attempt = 0; attempt < 5; attempt += 1) {
            const generatedKey = autoGeneratedKey(label.trim(), preferredKey, attempt);
            try {
              ...
              if (err instanceof ApiError && isKeyConflictError(err)) {
                lastKeyError = err;
                continue;
              }
              throw err;
          ...
          throw new ApiError({ message: "Couldn't generate a unique metric key." });
        };

        saved = await createWithAutoKey();
      }
