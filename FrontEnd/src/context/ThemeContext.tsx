...
type DynamicThemeMode = "idle" | "resolving" | "success" | "location-unavailable" | "api-failed";

interface DynamicThemeInfo {
...
const INITIAL_DYNAMIC_THEME_INFO: DynamicThemeInfo = {
  mode: "idle",
  source: null,
  sunrise: null,
  sunset: null,
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const resolveDynamicTheme = useCallback(async (): Promise<void> => {
    ...
    setDynamicThemeInfo((prev) => ({ ...prev, mode: "resolving" }));
  ...
  useEffect(() => {
    if (themePreference !== "dynamic") {
      clearDynamicTimer();
      setDynamicThemeInfo(INITIAL_DYNAMIC_THEME_INFO);
      return;
    }
  ...
  const value = useMemo(
    () => ({ theme, themePreference, setTheme, toggleTheme, dynamicThemeInfo }),
