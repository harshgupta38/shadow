...
import type { DateFormat, PlannerSettings, TimeFormat, WeekStartsOn } from "@/api/settings";
...
export function PlannerCard({
  ...
  function adjustDuration(delta: number) {
    const next = Math.max(5, Math.min(360, data.default_task_duration_minutes + delta));
      ...
      <FieldRow label="Default task duration" hint="Assumed length when creating a new task" className="pb-0">
        <div className="st-stepper">
          ...
          <button
            ...
            onClick={() => adjustDuration(5)}
            disabled={data.default_task_duration_minutes >= 360}
          >
            +
          </button>
        </div>
